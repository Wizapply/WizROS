...
void wzECANSetFilterRXB0(uint16_t sid, uint16_t mask)
{
    ...
    RXF0SIDL = sid << 5;
    RXF1SIDH = sid >> 3;    // filter 1, mask 0 -> RXB0
    RXF1SIDL = sid << 5;
...
{
    RXF2SIDH = sid >> 3;    // filter 2, mask 1 -> RXB1
    RXF2SIDL = sid << 5;
    RXF3SIDH = sid >> 3;    // filter 3, mask 1 -> RXB1
    RXF3SIDL = sid << 5;
    RXF4SIDH = sid >> 3;    // filter 4, mask 1 -> RXB1
    RXF4SIDL = sid << 5;
    RXF5SIDH = sid >> 3;    // filter 5, mask 1 -> RXB1
    RXF5SIDL = sid << 5;
...
void wzECANWrite(WIZ_CAN_PACKET* packet)
{
    ...
    if(PIE5bits.TXB0IE == 0) {
        ...
        //Data Set
        TXB0DLC = packet->dlc;
        TXB0SIDH = packet->id.sid >> 3;
        TXB0SIDL = packet->id.sid << 5;
...
//interrupt functions
void wzECANHandleRxInt() //RXB0 or RXB1
{
...
void wzECANHandleTxInt() //TXB0
{
    ...
    WIZ_CAN_PACKET* packet = &g_wzCANSendBuff[front];
...
void wzECANHandleTxErrInt() //TXB0 Error
{   
